...
namespace {
bool normalizeGridCodeInput(const std::string& inputCode, std::string& localCode, std::string& errorMsg) {
    if (inputCode.rfind("LGC1:", 0) == 0) {
        uint8_t level = 0;
        if (!parseInteropLocalCode(inputCode, localCode, level, errorMsg)) {
            errorMsg = "互操作编码无效: " + errorMsg;
            return false;
        }
        return true;
    }
...
void basicGrid::getGridByPoint(const HttpRequestPtr& req, std::function<void (const HttpResponsePtr &)> &&callback) const
{
    try {
        ...
        response["data"]["interopCode"] = toInteropLocalCode(gridCode, level);
...
void basicGrid::getCenterByGrid(const HttpRequestPtr& req, std::function<void (const HttpResponsePtr &)> &&callback) const
{
    try {
        ...
        // 调用局部网格解码函数获取中心点
        std::string localGridCode;
        std::string parseError;
        if (!normalizeGridCodeInput(gridCode, localGridCode, parseError)) {
            Json::Value response;
            response["status"] = "error";
...
void basicGrid::getChildCode(const HttpRequestPtr& req, std::function<void (const HttpResponsePtr &)> &&callback) const
{
    try {
        ...
        std::string localParentCode;
        std::string parseError;
        if (!normalizeGridCodeInput(parentCode, localParentCode, parseError)) {
            Json::Value response;
            response["status"] = "error";
            response["message"] = parseError;
            auto resp = HttpResponse::newHttpJsonResponse(response);
...
void basicGrid::getLocalToGlobal(const HttpRequestPtr& req, std::function<void (const HttpResponsePtr &)> &&callback) const
{
    try {
        ...
        std::string localCode = (*jsonBody)["localCode"].asString();
        if (localCode.empty()) {
            Json::Value response;
            response["status"] = "error";
        ...
        std::string normalizedLocalCode;
        std::string parseError;
        if (!normalizeGridCodeInput(localCode, normalizedLocalCode, parseError)) {
            Json::Value response;
            response["status"] = "error";
            response["message"] = parseError;
            auto resp = HttpResponse::newHttpJsonResponse(response);
...
void basicGrid::getGridCodeByPoint(const HttpRequestPtr& req, std::function<void (const HttpResponsePtr &)> &&callback) const
{
    try {
        ...
        response["interopCode"] = toInteropLocalCode(gridCode, level);
